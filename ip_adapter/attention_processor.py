...
class my_IPAttnProcessor2_0(torch.nn.Module):
    r"""
    Attention processor for IP-Adapater for PyTorch 2.0.
    Args:
        hidden_size (`int`):
    ...
    def __call__(
        ...
        self.denoise_step += 1
        # print(f"denoise_step:{self.denoise_step}")
        ...
        if encoder_hidden_states is None:
            ...
            # get encoder_hidden_states, ip_hidden_states
            end_pos = encoder_hidden_states.shape[1] - self.num_tokens
            # print(f"encoder_hidden_states.shape:{encoder_hidden_states.shape}")    # torch.Size([2, 85, 2048])
            encoder_hidden_states, ip_hidden_states = (
                encoder_hidden_states[:, :end_pos, :],                              # torch.Size([2, 77, 2048])
                encoder_hidden_states[:, end_pos:, :],     # torch.Size([2, 4, 2048])
            )
            if attn.norm_cross:
                encoder_hidden_states = attn.norm_encoder_hidden_states(encoder_hidden_states)
        ...
        query = query.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)        # torch.Size([2, 10 / 20, 4096, 64])
        ...
        if (not self.skip) and (self.denoise_step <= 20):
            ...
            def adain(content, style):
                ...
                # normalized_content = (content - content_mean)
                stylized_content = normalized_content * style_std + style_mean
                # stylized_content = normalized_content + style_mean
                return stylized_content

            ip_hidden_states1 = adain(content=hidden_states, style=ip_hidden_states)
            self.ip_hidden_states = ip_hidden_states1

            # hidden_states = hidden_states + self.scale * ip_hidden_states
            hidden_states = hidden_states + self.scale * self.ip_hidden_states
            # [2, 1024, 1280]
